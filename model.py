...
def process_webcam(fps=1):
    """
    Fonction pour traiter les images de la webcam et retourner les images annotées.
    Limite le nombre de FPS à `fps`.
    """

    # Initialiser la webcam
    cap = cv2.VideoCapture(0)
    
    # Définir la résolution de la webcam
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    
    # Optionnel : Définir une police pour le texte
    try:
        font = ImageFont.truetype("arial.ttf", size=15)
    except IOError:
        font = ImageFont.load_default()
    ...
    while True:
        ...
        # Convertir l'image annotée PIL en format OpenCV
        annotated_frame = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        
        # Attendre pour gérer les FPS
        if cv2.waitKey(int(1000 / fps)) & 0xFF == ord('q'):
            break

        # Renvoie l'image annotée pour affichage
